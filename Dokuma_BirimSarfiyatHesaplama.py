...
def load_data():
    if not os.path.exists(excel_path):
        st.error(f"❌ Excel dosyası bulunamadı! ({EXCEL_NAME})")
        return None
    try:
...
st.success("✅ Veri seti yüklendi. Tüm kriterler (Asorti dahil) birbirine bağlıdır.")
...
with col_left:
    ...
    # Bu adım sayesinde Asorti, yukarıdaki 4 seçime göre filtrelenir.
...
with col_right:
    ...
    # Güvenlik önlemi: Eğer filtreleme sonucu boş kalırsa (nadir durum), genel listeden getir
    if not asorti_list:
    ...
    # DÜZELTME: 'PARCA_SAYISI' (Hepsi büyük harf yapıldı)
    inputs['PARCA_SAYISI'] = st.number_input("PARCA_SAYISI", 1.0, 30.0, 18.0)

# -----------------------------------------------------------------------------
# 3. HESAPLAMA (OTOMATİK SIRALAMA DÜZELTMELİ)
# -----------------------------------------------------------------------------
st.divider()
...
if st.button("HESAPLA", type="primary", use_container_width=True):
    if model:
        try:
            ...
            st.error(f"Sütun Hatası: Model {e} isimli bir veri bekliyor ama kodda bu isim farklı yazılmış.")
        except Exception as e:
